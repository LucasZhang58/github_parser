...
def get_Org(full_repo_name, created_at, json_payload, record_d, in_dict, db):
        try:
        ...
        if user_id:
                org_dict['id'] =  user_id
        if avatar_url:
                org_dict['avatar_url'] = avatar_url
        if gravatar_id:
                org_dict['gravatar_id'] = gravatar_id
        if user_url:
                org_dict['url'] = user_url
        if user_company:
                org_dict['company'] = user_company
        if user_blog:
                org_dict['blog'] = user_blog
        if user_name:
                org_dict['name'] = user_name
        if user_location:
                org_dict['location'] = user_location
        if user_email:
                org_dict['email'] = user_email

        # save in the database
        try:
                db.add_org(user_login, org_dict)
        except Exception as e:
                print("Failed to save %s org data at %s: %s" % \
