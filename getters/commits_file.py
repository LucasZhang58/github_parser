...
def get_author_login_helper(in_dict):
        if 'actor' in in_dict:
...
def get_message_helper(in_dict):
        if 'commits' in in_dict:
                ...
                # commit_id
def get_commit_id_helper(in_dict):
        if 'commits' in in_dict:
                if in_dict['commits']:
                        if isinstance(in_dict['commits'][0], dict):
...
def get_PushEvent(repo_name, created_at, json_payload, record_d, db, commits, commit_past_repo_names):
        ...
        try:
                ...
                if 'actor' in record_d:
                        author_login = get_author_login_helper(record_d)
                elif 'actor' in json_payload:
                        author_login = get_author_login_helper(json_payload)
                ...
                # commit_id
                if 'shas' in json_payload:
                        commit_id = get_commit_id_helper(json_payload)
                elif 'shas' in record_d:
                        commit_id = get_commit_id_helper(record_d)
                elif 'commits' in json_payload:
                        commit_id = get_commit_id_helper(json_payload)
